...
Logger::Logger(string logger_name, const char *log_head_format,
		message_type_t loglevel, int default_log_level, const ostream &output) :
...
Logger::Logger(string logger_name, const char *log_head_format,
		message_type_t loglevel, int default_log_level, const char* filename) :
	ostream(new filebuf()) {
...
Logger::Logger(string logger_name, const char *log_head_format,
		message_type_t loglevel, int default_log_level, streambuf *obuf) :
	ostream(obuf) {
	this->logger_name = logger_name;
	this->loglevel = loglevel;
	this->local_timestamp = 0;
...
Logger *Logger::get_logger(string component, string format) {
	if (log_level_read == false)
		log_level_from_env = get_log_level_from_environment();
	if (loggers == 0)
		loggers = new map<string, Logger *> ;
	if (loggers->find(component) == loggers->end()) {
		loggers->insert(pair<string, Logger*> (component, new Logger(component,
				format.c_str(), log_level_from_env, 0, cout)));
	}
...
Logger *Logger::get_logger(string component, string format, string filename) {
	...
	if (loggers->find(component) == loggers->end()) {
		loggers->insert(pair<string, Logger*> (component, new Logger(component,
				format.c_str(), log_level_from_env, 0, filename.c_str())));
	}
	return loggers->find(component)->second;
}
...
Logger *Logger::get_logger(string component, string format, streambuf *obuf) {
	...
	if (loggers->find(component) == loggers->end()) {
		loggers->insert(pair<string, Logger*> (component, new Logger(component,
				format.c_str(), log_level_from_env, 0, obuf)));
	}
	return loggers->find(component)->second;
}
...
Logger *Logger::get_logger(string component) {
	...
	if (loggers->find(component) == loggers->end()) {
		loggers->insert(pair<string, Logger*> (component, new Logger(component,
				"[%l] %n@%a [%f@%L] - ", log_level_from_env, 0, cout)));
	}
	return loggers->find(component)->second;
}

void Logger::free_loggers() {
	for (map<string, Logger *>::iterator itL = loggers->begin(); itL
			!= loggers->end(); itL++) {
		delete itL->second;
	} // for
	delete loggers;
...
bool Logger::log_pct(double max, double current, double pct) {
	...
	if (current / tmp - floor(current / tmp) < 0.00001) {
		...
		cout << pnum << " - done: " << nearbyint((current / max * 100.0L))
				<< "%; prg size usage(MB): hodne moc" << endl;
		return true;
	}
	return false;
}
